    <!-- Footer -->
    <footer class="footer text-center">
      <h4> MAPA DEL SITIO </h4>
      <br />
      <div class="container">
      <div class="row">
      <div class="col-md-8 mb-5 mb-lg-0">
        <h6 class="text-uppercase mb-3"><a href="menuAdministrador.php">MENU ADMINISTRADOR</a></h6>
      </div>
      <div class="col-md-2 mb-5 mb-lg-0">
        <h6 class="text-uppercase mb-3"><a href="menuDocente.php">MENU DOCENTE</a></h6>
      </div>
      <div class="col-md-2 mb-5 mb-lg-0">
        <h6 class="text-uppercase mb-3"><a href="menuAlumno.php">MENU ALUMNOS</a></h6>
      </div>
      </div>
      </div>
      <div class="container">
        <div class="row">
          <div class="col-md-3 mb-5 mb-lg-0">        
            <ul class="list-unstyled">
              <li>
                <li><a href="altaAlumnos.php">Alta alumnos</a></li>
                <li><a href="bajaAlumnos.php">Baja alumnos</a></li>
                <li><a href="modificacionAlumnos.php">Modificación alumnos</a></li>
                <li><a href="listadoAlumnos.php">Listado de alumnos</a></li>
              </li>
            </ul>
          </div>
          <div class="col-md-2 mb-5 mb-lg-0">          
            <ul class="list-unstyled">
              <li>
                <li><a href="altaDocentes.php">Alta docentes</a></li>
                <li><a href="bajaDocentes.php">Baja docentes</a></li>
                <li><a href="modificacionDocentes.php">Modificación docentes</a></li>
                <li><a href="listadoDocentes.php">Listado docentes</a></li>
              </li>
            </ul>
          </div>
          <div class="col-md-3 mb-5 mb-lg-0">          
            <ul class="list-unstyled">
              <li>
                <li><a href="asignarMaterias.php">Asignar docentes a materias</a></li>
                <li><a href="asignarComisiones.php">Asignar docentes a comisiones</a></li>
              </li>
            </ul>
          </div>
          <div class="col-md-2 mb-5 mb-lg-0">
            <ul class="list-unstyled">
              <li><a href="altaNotas.php">Calificar alumnos</a></li>
              <li><a href="listadoComisiones.php">Listado de comisiones</a></li>
            </ul>
          </div>
          <div class="col-md-2 mb-5 mb-lg-0">
            <ul class="list-unstyled">
              <li><a href="inscripcionMaterias.php">Inscripción a materias</a></li>
              <li><a href="estadoAcademico.php">Estado académico</a></li>
            </ul>
          </div>
        </div>
      </div>
    </footer>

    <div class="copyright py-4 text-center text-white">
      <div class="container">
        <small>Zeballos 1341- C.P. 2000 - Rosario, Santa Fe, Argentina   //   Copyright &copy; Cardoso & Arnold - Entornos Gráficos 2018</small>
      </div>
    </div>